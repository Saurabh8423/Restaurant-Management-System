...
export default function Home() {
    ...
    useEffect(() => {
        api.get("/menu").then(res => {
            const data = res.data.menu || res.data || [];
            setItems(data);
            setVisibleItems((data.filter(i => i.category === selected)).slice(0, perPage));
        }).catch(() => {
            setItems(ITEMS);
            setVisibleItems(ITEMS.filter(i => i.category === selected).slice(0, perPage));
        });
    }, []);

    useEffect(() => {
        const filtered = items.filter(i => i.category === selected && i.name.toLowerCase().includes(search.toLowerCase()));
    ...
    const onAdd = (item, delta) => {
        setCart(prev => {
            ...
            if (exists) {
                const updated = prev.map(p => p.id === item.id ? { ...p, qty: Math.max(0, p.qty + delta) } : p).filter(p => p.qty > 0);
                return updated;
            } else if (delta > 0) {
                return [...prev, { id: item.id, name: item.name, price: item.price, qty: delta }];
            }
            return prev;
        });
    };
    ...
    return (
        <div className="home-shell">
            ...
            <div className="items-grid" onScroll={onScroll}>
                {visibleItems.map(it => <FoodCard key={it.id} item={it} qty={qtyOf(it.id)} onChangeQty={onAdd} />)}
            </div>
